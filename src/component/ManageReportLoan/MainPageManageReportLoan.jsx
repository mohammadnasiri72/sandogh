...
export default function MainPageManageReportLoan() {
  ...
  const config = {
    method: "get",
    url: `${mainDomain}/api/Report/Loan`,
    params:
  ...
  const getReportListLoan = async (newParams) => {
    ...
    await axios(config)
      .then((res) => {
        ...
        setListReportLoan(res.data.cooperativeLoanList);
      })
      .catch(() => {
        setIsLoading(false);
      });
  ...
  const getDataExcel = () => {
    ...
    axios
      .post(mainDomain + "/api/Report/Loan/Export", data, {
        headers: {
          Authorization: `Bearer ${user.token}`,
        },
      })
  ...
  const CustomFooter = ({ value = [], names = {}, format = "DD/MM/YYYY" }) => {
    return (
      <div className="rmdp-footer">
        {value.length > 0 ? (
          <div className="rmdp-footer-content">
            <span>{names.from}: {value[0].format(format)}</span>
            {value.length > 1 && (
              <span>
                {names.separator} {names.to}: {value[1].format(format)}
              </span>
            )}
          </div>
        ) : (
          <span>{names.selectDate}</span>
        )}
      </div>
    );
  };
