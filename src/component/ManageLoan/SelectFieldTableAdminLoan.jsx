...
SelectFieldTableAdminLoan.propTypes = {
  setSelectedField: PropTypes.func,
};
export default function SelectFieldTableAdminLoan({ setSelectedField }) {
  const valTab = useSelector((store) => store.adminLoan.valTab);

  const [selectedOptions, setSelectedOptions] = useState([]);
  useEffect(() => {
    if (valTab === 3) {
      setSelectedOptions([2, 3, 4, 6, 13]);
    } else if (valTab === 4 || valTab === 7) {
      setSelectedOptions([2, 3, 4, 9, 13]);
    } else if (valTab === 5) {
      setSelectedOptions([2, 3, 4, 6, 9, 13]);
    } else {
      setSelectedOptions([2, 3, 4, 13]);
    }
  }, [valTab]);
  useEffect(() => {
