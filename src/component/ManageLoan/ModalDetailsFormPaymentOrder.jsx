...
export default function ModalDetailsFormPaymentOrder({
  open,
  setOpen,
  flag,
  setFlag,
  ...
  const handleSubmit = () => {
    ...
    axios
      ...
      .then(() => {
        ...
        Toast.fire({
          icon: "success",
          text: "فرم دستور پرداخت با موفقیت ذخیره شد",
          customClass: {
  ...
  useEffect(() => {
    if (open) {
      setIsLoading(true);
      axios
        // .get(mainDomain + `/api/LoanForm/${loanId}/${3}`, {
        .get(mainDomain + `/api/Form/Loan/${loanId}/${3}`, {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        })
      ...
      >
        ...
        >
          <div className="px-4 flex gap-2">
            ...
            {status === 100 && LoanEdited?.formStatus >= 3 && (
              <ConfirmDeleteForm
                setFlagDel={setFlag}
                id={formId}
