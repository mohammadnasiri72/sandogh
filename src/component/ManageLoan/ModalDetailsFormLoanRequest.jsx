...
export default function ModalDetailsFormLoanRequest({
  open,
  setOpen,
  flag,
  setFlag,
  ...
  const handleSubmit = () => {
    ...
    axios
      ...
      .then(() => {
        ...
        Toast.fire({
          icon: "success",
          text: "فرم درخواست تسهیلات با موفقیت ذخیره شد",
          customClass: {
  ...
  useEffect(() => {
    if (open) {
      setIsLoading(true);
      axios
        // .get(mainDomain + `/api/LoanForm/${loanId}/${1}`, {
        .get(mainDomain + `/api/Form/Loan/${loanId}/${1}`, {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        })
      ...
      >
        ...
        >
          <div className="px-4 flex gap-2">
            ...
            {status === 100 && LoanEdited?.formStatus >= 1 && (
              <ConfirmDeleteForm
                setFlagDel={setFlag}
                id={formId}
