...
ModalUploadDoc.propTypes = {
  id: PropTypes.number,
};
export default function ModalUploadDoc({ id }) {
  ...
  const [valItemId, setValItemId] = React.useState(4406);

  const [uploadProgress, setUploadProgress] = React.useState(0);
  const user = JSON.parse(localStorage.getItem("user"));
  const cooperativeId = useSelector((store) => store.cooperative.cooperativeId);
  const optionItemId = [
    { id: 4406, title: "آگهی آخرین تغییرات" },
    { id: 4402, title: "فرم احراز هویت" },
    { id: 4407, title: "صورتهای مالی" },
    { id: 4409, title: "وکالت توثیق سهام" },
    { id: 4410, title: "آگهی تاسیس" },
    { id: 4408, title: "مدارک شناسایی اعضای هیئت مدیره" },
  ];
  ...
  const submitFormHandler = () => {
    ...
    if (fileSrc) {
      ...
      axios
        .post(mainDomain + "/api/CooperativeFile", data, {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        })
    ...
    <>
      ...
      >
        ...
        <div className=" flex flex-wrap items-start mt-5">
          <div className="sm:w-1/2 w-full px-2 sm:mt-0 mt-5">
          ...
          {!fileSrc && (
            <div className="flex justify-start px-2 items-start sm:w-1/2 w-full sm:mt-0 mt-3">
              {!isLoading && (
                <Button
                  size="large"
                  sx={{
          ...
          {fileSrc && (
            <div className="flex justify-center sm:w-1/2 w-full px-2 sm:mt-0 mt-3">
              <Chip
                className="truncate"
                label={fileName}
                // variant="outlined"
