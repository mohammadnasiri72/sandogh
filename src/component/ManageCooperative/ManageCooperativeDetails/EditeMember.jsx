...
EditeMember.propTypes = {
  setFlag: PropTypes.func,
  id: PropTypes.number,
};
export default function EditeMember({ setFlag, id }) {
  ...
  const [errMobile, setErrMobile] = React.useState("");
  ...
  const [memberEdited, setMemberEdited] = React.useState({});
  ...
  const handleChange = (event) => {
    ...
    setValposition(typeof value === "string" ? value.split(",") : value);
  };

  React.useEffect(() => {
    if (memberEdited.id) {
      setName(memberEdited.name);
      setFamily(memberEdited.family);
      setFatherName(memberEdited.fatherName);
      setPlaceBrith(memberEdited.placeBrith);
      setNationalCode(memberEdited.nationalCode);
      setShSh(memberEdited.shSh);
      setMobile(memberEdited.mobile);
      setEducation(memberEdited.education);
      setSignedRight(memberEdited.signedRight);
      setValposition(
        listposition.filter((item) =>
          memberEdited.positionIdList.split(",").map(Number).includes(item.id)
        )
      );
      setBrithDateFa(memberEdited.brithDateFa);
      setRegisterDateFa(memberEdited.registerDateFa);
      setValidityDateFa(memberEdited.validityDateFa);
      setDateFaBirth(
        memberEdited.brithDateFa
          ? new Date(
              moment(memberEdited.brithDateFa, "jYYYY/jMM/jDD").format(
                "YYYY-MM-DD"
              )
            )
          : ""
      ...
      setDateFaRegister(
        memberEdited.registerDateFa
          ? new Date(
              moment(memberEdited.registerDateFa, "jYYYY/jMM/jDD").format(
                "YYYY-MM-DD"
              )
            )
          : ""
      ...
      setDateFaValidity(
        memberEdited.validityDateFa
          ? new Date(
              moment(memberEdited.validityDateFa, "jYYYY/jMM/jDD").format(
                "YYYY-MM-DD"
              )
            )
          : ""
  ...
  React.useEffect(() => {
    if (open) {
      setLoader(true);
      Promise.all([
        axios.get(`${mainDomain}/api/BasicInfo/Position`, {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        }),
        axios.get(`${mainDomain}/api/Cooperative/Member/${id}`, {
          headers: {
            Authorization: `Bearer ${user.token}`,
          },
        }),
        ...
        .then((res) => {
          setLoader(false);
          setListposition(res[0].data);
          setMemberEdited(res[1].data);
        })
        .catch(() => {
          setLoader(false);
        });
    ...
    if (
      ...
      axios
        ...
        .then(() => {
          ...
          Toast.fire({
            icon: "success",
            text: "عضو فعلی با موفقیت ویرایش شد",
            customClass: {
  ...
  return (
    <>
      ...
      >
        ...
        >
          <span className="text-xl">ویرایش عضو فعلی</span>
