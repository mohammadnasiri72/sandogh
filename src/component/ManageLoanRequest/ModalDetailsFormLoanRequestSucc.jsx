...
ModalDetailsFormLoanRequestSucc.propTypes = {
  loan: PropTypes.object,
};
export default function ModalDetailsFormLoanRequestSucc({ loan }) {
  ...
  useEffect(() => {
    if (open) {
      setIsLoading(true);
      axios
        // .get(mainDomain + `/api/Form/Loan/${loan.loanId}/${1}`, {
        .get(mainDomain + `/api/Form/Request/${loan.id}/${1}`, {
          headers: {
    ...
    <>
      <div>
        ...
        >
          ...
          <span className="text-[#fff]">فرم درخواست تسهیلات</span>
        </Button>
      </div>
      <Dialog
        fullScreen
      ...
      >
        ...
        <div className="">
          <div className="px-2 mt-20">
            ...
            {!isLoading && !formDetails && errMessage && (
              <span>
                {typeof errMessage === "string" ? errMessage : "آیتمی یافت نشد"}
              </span>
            )}
            {!isLoading && !formDetails && !errMessage && (
              <span>آیتمی یافت نشد</span>
            )}
          </div>
        </div>
      </Dialog>
