...
ShowFacilities.propTypes = {
  status: PropTypes.number,
  loanId: PropTypes.number,
};

export default function ShowFacilities({ status, loanId }) {
  ...
  useEffect(() => {
    if (loanId && open) {
      setLoader(true);
      axios
        .get(mainDomain + `/api/Loan/${loanId}`, {
          headers: {
  ...
  return (
    <>
      <div className="px-1">
        <Tooltip
          title={
            status !== 102 ? "نمایش تسهیلات ثبت شده" : "نمایش تسهیلات معوق"
          }
        >
      ...
      >
        <div
          ...
          className={
            themeMode === "dark"
              ? "flex justify-between px-5 items-center py-2"
              : "flex justify-between px-5 items-center py-2"
          }
        >
          <span className="text-2xl">{LoanEdited.cooperativeTitle}</span>

          <IconButton onClick={handleClose}>
            <CgClose style={{ color: themeColor.color }} className="text-xl" />
          </IconButton>
        </div>
        <>
          <div className="px-1 flex justify-start mt-2">
            <Button
              onClick={() => {
                handleClose();
              }}
          ...
          <div className="flex items-center justify-between p-4 flex-wrap">
            {/* <h4 className="sm:w-1/4 w-full sm:mt-0 mt-5">
              {LoanEdited.cooperativeTitle}
            </h4> */}
            <div className="sm:w-1/3 w-full">
              <span>{LoanEdited.requestTitle}-</span>
              <span>کد: </span>
              <span>{LoanEdited.loanRequestId}</span>
            </div>
          ...
          <div>
            <div className="mt-7 px-3 flex flex-wrap items-start">
              ...
              <div className="relative md:w-1/5 w-full px-2 mt-5">
                <TextField
                  size="small"
                  label="مدت قرارداد"
                  className="border rounded-lg w-full px-3"
              ...
              <div className="flex relative md:w-1/5 w-full px-2 mt-5">
                <TextField
                  ...
                  FormHelperTextProps={{ sx: { color: "red" } }}
                  helperText={
                    errAmountValue ? "لطفا مقدار نهاده را وارد کنید" : ""
                  }
                  sx={{
                    "& .MuiOutlinedInput-root": {
                      "& fieldset": {
              ...
              <div className="md:w-1/5 w-full px-2 mt-5">
                <TextField
                  ...
                  FormHelperTextProps={{ sx: { color: "red" } }}
                  helperText={
                    errUnitPrice ? "لطفا قیمت هر کیلو را وارد کنید" : ""
                  }
                  sx={{
                    "& .MuiOutlinedInput-root": {
                      "& fieldset": {
